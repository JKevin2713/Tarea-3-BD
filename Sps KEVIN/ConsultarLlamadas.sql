...
/****** Object:  StoredProcedure [dbo].[ConsultarLlamadas]    Script Date: 15/06/2024 15:43:31 ******/
SET ANSI_NULLS ON
GO
...
--------------------------------------------------------------------
CREATE PROCEDURE [dbo].[ConsultarLlamadas]
    @InNumero BIGINT,
    ...
    BEGIN TRY
			...
			BEGIN 
				...
				-- Seleccionar las llamadas que ocurrieron dentro del rango deseado
					...
					L.NumeroDe,
					L.NumeroA,
					CONVERT(VARCHAR(8), L.Inicio, 108) AS Inicio,
					CONVERT(VARCHAR(8), L.Fin, 108) AS Fin,
					L.FechaOperacion,
					DATEDIFF(MINUTE, L.Inicio, L.Fin) AS DuracionMinutos
				FROM 
					LlamadaTelefonica L
				WHERE 
					(L.NumeroDe = @InNumero OR L.NumeroA = @InNumero)
					AND L.Inicio >= @Fechainicio
					AND L.Inicio <= @FechaFin
				ORDER BY 
					L.Inicio ASC;
			END
