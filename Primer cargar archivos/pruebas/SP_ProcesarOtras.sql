ALTER PROCEDURE ProcesarLlamadasOtro
AS
BEGIN
    SET NOCOUNT ON;
    ...
    WHERE LEFT(CAST(NumeroDe AS VARCHAR(20)), 3) != '800'; -- Evita las llamadas con receptor 800
    ...
    BEGIN
        -- Verificar si el número receptor no es 800
        IF LEFT(CAST(@NumeroA AS VARCHAR(20)), 3) != '800'
        BEGIN
            -- Obtener el contrato asociado con el número receptor
            SELECT @TipoTarifa = c.TipoTarifa
            FROM Contratos c
            WHERE c.Numero = @NumeroA;
            ...
            -- Insertar información de la llamada en la tabla LlamadasOtro
            INSERT INTO LlamadasOtro (FechaOperacion, DuracionMinutos, Emisor, Receptor, ValorAntesMultiplicar, ValorMultiplicado)
...
EXEC ProcesarLlamadasOtro
SELECT * FROM LlamadasOtro
